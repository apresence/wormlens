...
def _parse_chat_session(
    ...
    def flush():
        ...
        text = _unescape_chat_content(text).strip()
        if not text:
            return
        msg_type = cur_subtype or "msg"
    ...
    for raw in lines:
        ...
        if m:
            flush()
            cur_role = m.group(1)
            cur_subtype = m.group(2)  # e.g. "team_msg" for assistant/team_msg
            try:
